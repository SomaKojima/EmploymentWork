...
bool Collision::HitCheck_Segment_Plane(Segment & segment, Plane & plane, Vector3 * hit_pos)
{
	// 線分始点と平面の距離を計算
...
bool Collision::HitCheck_Segment_Sphere(Segment & segment, Sphere & sphere, Vector3 * hit_pos)
{
	// (2次方程式) = at^2 + bt + c = 0;
	// 2次方程式におけるa,b,cを計算
...
bool Collision::HitCheck_Segment_Triangle(Segment & segment, Triangle & triangle, Vector3 * hit_pos)
{
	// 三角形の平面とチェック
	Vector3 tmp_hit_pos;
...
bool Collision::Triangle_CheckInner(Triangle & triangle, Vector3& point)
{
	// 任意点から各頂点へのベクトル作成
...
bool Collision::HitCheck_Sphere_Plane(Sphere & sphere, Plane & plane, Vector3* hit_pos)
{
...
bool Collision::HitCheck_Sphere_Triangle(Sphere & sphere, Triangle & triangle, Vector3 * hit_pos)
{
	...
	if (HitCheck_Segment_Sphere(segment, sphere, hit_pos)) return true;
	...
	if (HitCheck_Segment_Triangle(segment, triangle, hit_pos)) return true;

	return false;
}

bool Collision::HitCheck_Sphere_Triagnles(Triangle triangle[], int triangle_index, Sphere & sphere, Vector3 * hit_pos)
{
...
bool Collision::HitCheck(Entity * entity, Entity* entity2, CollisionData* data)
{
	...
	if (sphereCollision)
	{
		data->component1 = sphereCollision;
		if (sphereCollision2)
		{
			data->component2 = sphereCollision2;
			return HitCheck_Sphere(entity, *sphereCollision, entity2, *sphereCollision2, &data->hit_pos);
		}
		if (planeCollision2)
		{
			data->component2 = planeCollision2;
			return HitCheck_Sphere_Triagnles(planeCollision2->GetTriangle(), planeCollision2->GetTriangleIndex(), sphereCollision->GetSphere(), &data->hit_pos);
		}
	}
	if (planeCollision)
	{
		data->component1 = planeCollision;
		if (sphereCollision2)
		{
			data->component2 = sphereCollision2;
			return HitCheck_Sphere_Triagnles(planeCollision->GetTriangle(), planeCollision->GetTriangleIndex(), sphereCollision2->GetSphere(), &data->hit_pos);
		}
	}
